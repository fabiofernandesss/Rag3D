...
@pytest.mark.parametrize(
    ...
    [
        _SENSITIVE_DETAIL,
        _SENSITIVE_DETAIL + "-driver",
        _SENSITIVE_DETAIL + "-encoded",
    ],
)
...
def test_docker_pgvector_profile_is_pinned_and_legacy_service_is_preserved() -> None:
    ...
    legacy, vector = compose.split("  pgvector:", 1)
    password_key = "POSTGRES_" + "PASSWORD"
    assert "  db:" in legacy
    assert "image: postgres:16" in legacy
    assert "container_name: rag3d-pg" in legacy
    assert f"{password_key}:" in legacy
    assert '"5433:5432"' in legacy
    assert "rag3d_pg:/var/lib/postgresql/data" in legacy
    ...
    assert f"{password_key}: ${{RAG3D_PGVECTOR_PASSWORD" in vector
    assert "RAG3D_PGVECTOR_PASSWORD:?" not in compose
    assert f"{password_key}: rag3d" not in vector
